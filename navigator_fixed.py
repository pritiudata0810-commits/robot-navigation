...
class Navigator:
    ...
    def show_main_menu(self) -> GameConfig:
        ...
        difficulty_panel = Panel(
            ...
            "[cyan]Easy[/]     - 10 percent walls\n"
            "[yellow]Medium[/]  - 25 percent walls\n"
            "[#FF0000]Hard[/]     - 40 percent walls",
            border_style="#00FF00",
            padding=(1, 2)
        )
        ...
        speed_panel = Panel(
            ...
            "[cyan]Slow[/]    - 1.0s per step\n"
            "[yellow]Normal[/]  - 0.5s per step\n"
    ...
    def render_grid(self) -> str:
        ...
        for y in range(self.grid.size):
            ...
            for x in range(self.grid.size):
                ...
                # Apply style with universal closing tag
                if style:
                    first_style = style.split()[0]
                    line += f"[{style}]{char}[/]"
                else:
                    line += char
    ...
    def run_animation(self) -> None:
        ...
        # Create a wrapper render function to include telemetry
    ...
    def _show_success_celebration(self) -> None:
        ...
        summary = Table(title="[bold #00FF00]MISSION SUMMARY[/]", border_style="#FF00FF")
        ...
        self.console.print()
        self.console.print("[bold #00FF00]✓ MISSION OBJECTIVE COMPLETE ✓[/]", justify="center")
    ...
    def run(self) -> None:
        ...
        try:
            ...
            if not self.game_state.path:
                self.console.print("[bold red]❌ SYSTEM ERROR: [/]No valid path found!")
                return
            
            self.console.print(f"[green]✓ Path found with {len(self.game_state.path)} steps![/]")
            self.console.print("[yellow]Press Enter to start animation...[/]")
            input()
            ...
            # Ask to play again
            self.console.print()
            if Confirm.ask("[bold]Play again?[/]", default=True):
                self.run()
            else:
                self.console.print("[cyan]👋 Thanks for playing![/]")
